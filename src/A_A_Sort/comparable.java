...
public class comparable{
	public static void main(String[] args) {
		...
		sortedByRating.add(new movie("Bhaubali" ,4, 2027 ));
		sortedByRating.add(new movie("Bhaubali2" ,5, 2023 ));
		sortedByRating.add(new movie("Racha" ,1, 2025 ));
		sortedByRating.add(new movie("ab" ,4, 2023 ));
...
class movie implements Comparable<movie> {
	...
	public int compareTo(movie obj) {
		int relYear = this.releaseYear - obj.releaseYear ;
		//return 1 if it is greater, -1 if it is less , 0 for equal
		if(relYear!=0)
			return relYear;	
